...
const Add = ({ token }) => {
  // -------- Atualização de imagens de produto existente --------
  const [products, setProducts] = useState([]);
  const [selectedProductId, setSelectedProductId] = useState("");
  const [updImage1, setUpdImage1] = useState(false);
  const [updImage2, setUpdImage2] = useState(false);
  const [updImage3, setUpdImage3] = useState(false);
  const [updImage4, setUpdImage4] = useState(false);
  ...
  const handleUpdateImages = async () => {
    try {
      ...
      if (!selectedProductId) {
        toast.error("Selecione um produto para atualizar as imagens.");
        return;
      }

      const files = [updImage1, updImage2, updImage3, updImage4].filter(Boolean);
      if (!files.length) {
        toast.error("Selecione ao menos uma nova imagem.");
        return;
      }

      const fd = new FormData();
      // força limpar imagens antigas
      fd.append("keepImages", JSON.stringify([]));
      files.forEach((file, idx) => fd.append(`image${idx + 1}`, file));

      const { data } = await axios.put(
        `${backendUrl}/api/product/update/${selectedProductId}`,
        fd,
        {
          headers: {
            token,
      ...
      if (data?.success) {
        toast.success("Imagens do produto atualizadas!");
        setUpdImage1(false);
        setUpdImage2(false);
        setUpdImage3(false);
        setUpdImage4(false);
      ...
      toast.error(error?.response?.data?.message || "Erro ao atualizar imagens.");
    }
  };
      ...
      <section className="rounded-2xl border border-gray-200 p-4 bg-gray-50">
        <div className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <p className="font-semibold text-gray-800">Atualizar imagens de um produto existente</p>
              <p className="text-sm text-gray-600">
                Selecione um produto e envie novas imagens. As imagens antigas serão substituídas.
              </p>
            </div>
            <button
              type="button"
              onClick={handleUpdateImages}
              className="px-4 py-2 rounded-lg bg-black text-white hover:opacity-90 text-sm font-semibold"
            >
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
            <label className="text-sm font-medium text-gray-700" htmlFor="select-product-images">
              Produto
            </label>
            <select
              id="select-product-images"
              value={selectedProductId}
              onChange={(e) => setSelectedProductId(e.target.value)}
              className="w-full sm:w-auto flex-1 px-3 py-2 border rounded"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[updImage1, updImage2, updImage3, updImage4].map((img, index) => {
              const setters = [setUpdImage1, setUpdImage2, setUpdImage3, setUpdImage4];
              const setImg = setters[index];
              return (
                <Motion.label
                  htmlFor={`upd-image${index + 1}`}
                  key={`upd-${index}`}
                  className="w-full h-28 sm:h-32 border-2 border-dashed border-gray-300 rounded-lg overflow-hidden cursor-pointer bg-white hover:bg-gray-50 hover:shadow transition-all flex items-center justify-center"
